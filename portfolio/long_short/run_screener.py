...
try:
    from config import (
        ...
        RESULTS_DIR, PORTFOLIO_BUY_DIR, PORTFOLIO_SELL_DIR
    )
    print("\n✅ config.py 모듈 임포트 성공")
    ...
    print(f"- PORTFOLIO_BUY_DIR: {PORTFOLIO_BUY_DIR}")
    print(f"- PORTFOLIO_SELL_DIR: {PORTFOLIO_SELL_DIR}")
    ...
    for path_name, path in [
        ...
        ("PORTFOLIO_BUY_DIR", PORTFOLIO_BUY_DIR),
        ("PORTFOLIO_SELL_DIR", PORTFOLIO_SELL_DIR),
    ]:
    ...
    buy_dir = PORTFOLIO_BUY_DIR
    sell_dir = PORTFOLIO_SELL_DIR
...
# long_short 포트폴리오 디렉토리 확인
long_short_dir = root_dir
print(f"\n📁 long_short 포트폴리오 디렉토리: {long_short_dir}")
...
try:
    # 개별 포트폴리오 관리는 portfolio.manager 모듈을 통해 실행됩니다.
    ...
    print("   개별 포트폴리오 관리는 portfolio.manager 모듈을 사용하세요.")

    # 결과 파일 확인 (예시로 strategy1과 strategy2만 확인)
    print("\n📄 결과 파일 생성 확인 (일부 전략):")
    strategy1_result_file = os.path.join(PORTFOLIO_BUY_DIR, 'strategy1_results.csv')
    strategy2_result_file = os.path.join(PORTFOLIO_SELL_DIR, 'strategy2_results.csv')
