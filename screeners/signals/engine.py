...
_SOURCE_SPECS: tuple[_SourceSpec, ...] = (
    _SourceSpec("leader_lagging/leaders.csv", "LL_LEADER", "LEADER", True),
    _SourceSpec("leader_lagging/followers.csv", "LL_FOLLOWER", "FOLLOWER", True),
    _SourceSpec(
    ...
    _SourceSpec(
        "markminervini/integrated_results.csv", "MM_INTEGRATED", "INTEGRATED", True
    ),
    _SourceSpec(
        "tradingview/us_breakout_rvol.csv", "TV_BREAKOUT_RVOL", "TV_BREAKOUT_RVOL", True
    ),
    _SourceSpec(
        "tradingview/us_breakout_10m.csv", "TV_BREAKOUT_10M", "TV_BREAKOUT_10M", True
    ),
    _SourceSpec(
    ...
    _SourceSpec(
        "tradingview/us_market_leader.csv", "TV_MARKET_LEADER", "TV_MARKET_LEADER", True
    ),
    _SourceSpec(
    ...
    _SourceSpec(
        "tradingview/kr_breakout_rvol.csv", "TV_BREAKOUT_RVOL", "TV_BREAKOUT_RVOL", True
    ),
    _SourceSpec(
        "tradingview/kr_market_leader.csv", "TV_MARKET_LEADER", "TV_MARKET_LEADER", True
    ),
    _SourceSpec(
        "weinstein_stage2/primary_candidates.csv", "WS_PRIMARY", "WS_PRIMARY", True
    ),
    _SourceSpec(
    ...
    _SourceSpec(
        "weinstein_stage2/retest_candidates.csv", "WS_RETEST", "WS_RETEST", True
    ),
    _SourceSpec(
    ...
    _SourceSpec("qullamaggie/daily_focus_list.csv", "QMG_DAILY", "DAILY_FOCUS", True),
    _SourceSpec(
        "qullamaggie/weekly_focus_list.csv", "QMG_WEEKLY", "WEEKLY_FOCUS", True
    ),
    _SourceSpec(
        "qullamaggie/universe_list.csv", "QMG_UNIVERSE", "UNIVERSE_WATCH", False
    ),
)
...
def _stage_priority(stage: str) -> int:
    return _STAGE_PRIORITY.get(str(stage or "").strip(), 0)


def _source_tag_priority(tag: str) -> float:
    return float(_SOURCE_TAG_PRIORITY.get(_safe_text(tag), 0.0))


def _source_tag_style(tag: str) -> str:
    return _safe_text(_SOURCE_TAG_STYLE.get(_safe_text(tag), "WATCH"))


def _source_style_bonus(style: str, *, engine: str) -> float:
    normalized_engine = _safe_text(engine).upper()
    normalized_style = _safe_text(style).upper()
    if normalized_engine == "UG":
        return float(_UG_STYLE_BONUS.get(normalized_style, 0.0))
    return float(_TREND_STYLE_BONUS.get(normalized_style, 0.0))


def _sorted_source_tags(tags: Iterable[str] | object | None) -> list[str]:
    unique_tags = list(dict.fromkeys(_to_list(tags)))
    return sorted(unique_tags, key=lambda tag: (-_source_tag_priority(tag), tag))


def _source_priority_score(tags: Iterable[str]) -> float:
    sorted_tags = _sorted_source_tags(tags)
    if not sorted_tags:
        return 0.0
    top_two = sorted_tags[:2]
    overlap_bonus = max(len(sorted_tags) - 1, 0) * 1.5
    return round(
        min(sum(_source_tag_priority(tag) for tag in top_two) + overlap_bonus, 20.0), 2
    )
...
def _source_engine_bonus(tags: Iterable[str], *, engine: str) -> float:
    ...
    return round(
        min(
            sum(
                _source_style_bonus(_source_tag_style(tag), engine=engine)
                for tag in top_two
            ),
            10.0,
        ),
...
def _build_signal_row(
    *,
    ...
    deprecated_alias_code: str = "",
    deprecated_alias_reason: str = "",
    ...
    return {
        ...
        "contract_version": _safe_text(contract_version),
        "deprecated_alias_code": _safe_text(deprecated_alias_code),
        "deprecated_alias_reason": _safe_text(deprecated_alias_reason),
...
def _legacy_signal_row(row: Mapping[str, Any]) -> dict[str, Any]:
    updated = dict(row)
    updated["contract_version"] = _CONTRACT_VERSION_LEGACY
...
def _transform_signal_rows(
    ...
    *,
    ...
    for row in rows:
        if contract_version == _CONTRACT_VERSION_V2:
            updated = _v2_signal_row(row)
            if updated is not None:
                transformed.append(updated)
        else:
            transformed.append(_legacy_signal_row(row))
...
def _legacy_cycle_row(row: Mapping[str, Any]) -> dict[str, Any]:
    updated = dict(row)
    updated["contract_version"] = _CONTRACT_VERSION_LEGACY
    updated["deprecated_alias_code"] = ""
    updated["deprecated_alias_reason"] = ""
...
class MultiScreenerSignalEngine:
            ...
            rows.append(
                {
                    ...
                    "sector": _safe_text(source_entry.get("sector")),
                    "industry": _safe_text(source_entry.get("industry")),
                    "group_name": _safe_text(source_entry.get("group_name")),
    ...
    def run(self) -> dict[str, Any]:
        ...
        legacy_trend_event_rows = _transform_signal_rows(
            trend_event_rows, contract_version=_CONTRACT_VERSION_LEGACY
        )
        legacy_trend_state_rows = _transform_signal_rows(
            trend_state_rows, contract_version=_CONTRACT_VERSION_LEGACY
        )
        legacy_ug_event_rows = _transform_signal_rows(
            ug_event_rows, contract_version=_CONTRACT_VERSION_LEGACY
        )
        legacy_ug_state_rows = _transform_signal_rows(
            ug_state_rows, contract_version=_CONTRACT_VERSION_LEGACY
        )
        legacy_all_signal_rows = _sorted_signal_rows(
            legacy_trend_event_rows
            + legacy_trend_state_rows
            + legacy_ug_event_rows
            + legacy_ug_state_rows
        )

        trend_event_rows_v2 = _apply_update_overlay_rows(
            _transform_signal_rows(
                trend_event_rows, contract_version=_CONTRACT_VERSION_V2
            ),
            metrics_map,
        )
        trend_state_rows_v2 = _apply_update_overlay_rows(
            _transform_signal_rows(
                trend_state_rows, contract_version=_CONTRACT_VERSION_V2
            ),
            metrics_map,
        )
        ug_event_rows_v2 = _apply_update_overlay_rows(
            _transform_signal_rows(
                ug_event_rows, contract_version=_CONTRACT_VERSION_V2
            ),
            metrics_map,
        )
        ug_state_rows_v2 = _apply_update_overlay_rows(
            _transform_signal_rows(
                ug_state_rows, contract_version=_CONTRACT_VERSION_V2
            ),
            metrics_map,
        )
        ug_combo_rows_v2 = _apply_update_overlay_rows(
            _transform_signal_rows(
                ug_combo_rows, contract_version=_CONTRACT_VERSION_V2
            ),
            metrics_map,
        )
        ...
        open_cycle_rows_legacy = _sorted_signal_rows(
            _legacy_cycle_row(row) for row in updated_cycles.values()
        )
        ...
        summary = {
            ...
            "as_of_date": self.as_of_date,
            "counts": {
                "trend_events": len(legacy_trend_event_rows),
                "trend_states": len(legacy_trend_state_rows),
                "ug_events": len(legacy_ug_event_rows),
                "ug_states": len(legacy_ug_state_rows),
                ...
                "deprecated_rows": sum(
                    1
                    for row in legacy_all_signal_rows
                    if _safe_text(row.get("deprecated_alias_reason"))
                ),
            },
        }
        ...
        return {
            ...
            "ultimate_growth_events": legacy_ug_event_rows,
            "ultimate_growth_states": legacy_ug_state_rows,
            "all_signals": legacy_all_signal_rows,
            ...
            "open_family_cycles_legacy": open_cycle_rows_legacy,
