...
export default function IPOInvestmentPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await apiClient.getIPOInvestmentResults();
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="text-lg">Loading IPO results...</div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <h1 className="text-3xl font-bold text-gray-800 mb-6">IPO Investment Screener</h1>
      {selectedSymbol && (
      ...
      {data.length > 0 ? (
        <DataTable data={data} columns={columns} onSymbolSelect={setSelectedSymbol} disableModal />
      ) : (
